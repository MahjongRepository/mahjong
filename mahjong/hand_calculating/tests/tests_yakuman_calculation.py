...
class YakumanCalculationTestCase(unittest.TestCase, TestMixin):
    ...
    def test_disabled_double_yakuman(self):
        ...
        result = hand.estimate_hand_value(
            tiles, win_tile, config=self._make_hand_config(is_tsumo=True, disable_double_yakuman=True)
        )
        self.assertEqual(result.error, None)
        self.assertEqual(result.han, 13)
        self.assertEqual(result.fu, 50)
    ...
    def test_is_daichikurin(self):
        ...
        result = hand.estimate_hand_value(
            tiles, win_tile, config=self._make_hand_config(allow_daisharin=True, allow_daisharin_other_suits=True)
        )
        self.assertEqual(result.error, None)
        self.assertEqual(result.han, 13)
        self.assertEqual(len(result.yaku), 1)
    ...
    def test_is_open_riichi_sashikomi(self):
        ...
        result = hand.estimate_hand_value(
            tiles,
            win_tile,
            config=self._make_hand_config(is_riichi=True, is_open_riichi=True, has_sashikomi_yakuman=True),
        )
        self.assertEqual(result.error, None)
        self.assertEqual(result.han, 13)
        self.assertEqual(len(result.yaku), 1)
    ...
    def test_paarenchan_no_yaku_allowed(self):
        ...
        win_tile = self._string_to_136_tile(pin="7")
        result = hand.estimate_hand_value(
            tiles, win_tile, config=self._make_hand_config(is_tsumo=False, paarenchan=1, paarenchan_needs_yaku=False)
        )
