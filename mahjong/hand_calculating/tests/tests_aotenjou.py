...
class AotenjouCalculationTestCase(unittest.TestCase, TestMixin):
    ...
    def test_aotenjou_hands(self):
        ...
        result = hand.estimate_hand_value(tiles, win_tile, scores_calculator_factory=Aotenjou,
                                          config=self._make_hand_config(player_wind=EAST, round_wind=EAST,
        ...
        result = hand.estimate_hand_value(tiles, win_tile, melds=melds, scores_calculator_factory=Aotenjou,
        ...
        result = hand.estimate_hand_value(tiles, win_tile, melds=melds, scores_calculator_factory=Aotenjou,
                                          config=self._make_hand_config(is_tsumo=True, player_wind=EAST,
                                                                        round_wind=EAST))
        self.assertEqual(result.error, None)
        ...
        result = hand.estimate_hand_value(tiles, win_tile, melds=melds,
                                          ...
                                          config=self._make_hand_config(is_riichi=True, is_tsumo=True,
                                                                        is_ippatsu=True, is_haitei=True,
                                                                        player_wind=EAST, round_wind=EAST))
        self.assertEqual(result.error, None)
    ...
    def test_chuuren_poutou(self):
        ...
        result = hand.estimate_hand_value(tiles, win_tile, scores_calculator_factory=Aotenjou,
                                          config=self._make_hand_config(is_tsumo=True, player_wind=EAST,
                                                                        round_wind=EAST))
        self.assertEqual(result.error, None)
        self.assertEqual(result.han, 29)
        self.assertEqual(result.fu, 30)
        self.assertEqual(len(result.yaku), 3)
        self.assertEqual(result.cost['main']+result.cost['additional'], 257698037800)
    ...
    def test_daisharin(self):
        ...
        result = hand.estimate_hand_value(tiles, win_tile, scores_calculator_factory=Aotenjou,
                                          config=self._make_hand_config(is_tsumo=True, player_wind=EAST,
                                                                        round_wind=EAST, allow_daisharin=True))
        self.assertEqual(result.error, None)
