...
Wind yakuhai are now represented as individual yaku classes per direction: `SeatEast`, `SeatSouth`, `SeatWest`, `SeatNorth` for seat winds, and `RoundEast`, `RoundSouth`, `RoundWest`, `RoundNorth` for round winds.
...
- Wind yakuhai classes have been renamed and restructured:
  - `YakuhaiEast` → `SeatEast`, `YakuhaiSouth` → `SeatSouth`, `YakuhaiWest` → `SeatWest`, `YakuhaiNorth` → `SeatNorth`. Each now checks only the seat wind condition.
  - `YakuhaiOfPlace` and `YakuhaiOfRound` have been removed and replaced by per-direction round wind classes: `RoundEast`, `RoundSouth`, `RoundWest`, `RoundNorth`.
